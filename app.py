...
"""
        ...
        cat_sub_links = (
            questions_df
                .drop_duplicates(subset=["Category", "Sub-Category"])[["Category", "Sub-Category"]]
                .copy()
        )
        cat_sub_links.loc[:, "value"] = 2  # thicker connections

        # Subcategory -> Question
        sub_q_links = questions_df[["Sub-Category", "Question", "Question Score"]].copy()

        # Ensure numeric + minimum value for visualization
        sub_q_links["Question Score"] = (
            pd.to_numeric(sub_q_links["Question Score"], errors="coerce")
            .fillna(0)
            .clip(lower=0.1)
        )
        ...
        # === Create gradient link colors based on source node (example placeholder) ===
        link_colors = []
